...
class Classes {
     ...
     *             Thrown when something very odd is happening with classloaders.
     */
    // @NotNull
    public Set<Class<?>> in(Package pack) {
        ...
        while (dirs.hasMoreElements()) {
            ...
            if ("file".equals(protocol)) {
                try {
                    findClassesInDirPackage(packageOnly, URLDecoder.decode(url.getFile(), "UTF-8"),
                            recursive, classes);
                    ...
                    if (name.startsWith(packageDirName)) {
                        ...
                        if ((idx != -1) || recursive) {
                            ...
                            if (name.endsWith(".class") && !entry.isDirectory()) {
                                ...
                                /*
                                 * Issue #26 - package-info causes ISE during package scan. This ia a bit of a
                                 * hack for just package-info. TODO Determine better handling of unexpected
                                 * classloader issues.
                                 */
    ...
    private void findClassesInDirPackage(String packageName, String packagePath, final boolean recursive,
            Set<Class<?>> classes) {
        ...
        for (File file : dirfiles) {
            if (file.isDirectory()) {
                findClassesInDirPackage(packageName + "." + file.getName(), file.getAbsolutePath(),
                        recursive, classes);
            } else {
